...
Certainly looks like there is a specific amount of time (around 15 minutes) of delay that is hidden by the second histogram.
...
Daniel - There are 68 flights that match this criteria
```{r}
...
Daniel - It looks like DL or VX would have the most variable since its median is -10 or worse but its IQR is so far from it at around 22 for both.
```{r}
sfo_feb_flights %>%
  group_by(carrier) %>%
...
Daniel - Looks like the answer is going to be July.
...
Daniel - It depends on whether there is high variance in that month. If there is high variance, you would want to pick the one with the lower median since its less sensitive to outliers. The average would be better if there is less variance.
...
Daniel - I would choose the LGA airport since its on time departure rate is 72%
...
Daniel - The relationship looks exponential
```{r}

ggplot(data = nycflights, aes(x = avg_speed, y = distance, color = carrier)) +
  geom_point()
...
ggplot(data = dl_aa_ua, aes(x = dep_delay, y = arr_delay, color = carrier)) +
  geom_point()
```
Daniel - Note sure if I am understanding the request correctly, but I made two plots, one that had all flights and it looks like you're definitely not going to make it on time if you leave past 50 minutes original departure time (even then its rare) when looking at all flights. I added x and y lims to zoom in on the area in question.

I also made another plot with different carriers.

```{r}
ggplot(data = nycflights, aes(x = dep_delay, y = arr_delay, color = carrier)) +
  geom_point() + xlim(-15, 500) + ylim(-15,500)


```

Daniel - If we restrict this to just three carriers it, I chose VX, 9E, and HA, you could potentially get there on time even if you left 30 minutes after original departure time. I know this isn't an exact "replica", I put limits in to zoom in a bit more on the area in question (0 arrival delay)

```{r}
unique(nycflights$carrier)

vx_9e_ha <- nycflights %>%
  filter(carrier == "VX" | carrier == "9E" | carrier == "HA")
ggplot(data = vx_9e_ha, aes(x = dep_delay, y = arr_delay, color = carrier)) +
  geom_point() + xlim(-15, 500) + ylim(-15,500)
```
